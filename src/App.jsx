...
// ── CONSTANTS ──────────────────────────────────────────────────────────────────
const BANK = { bank: "SeaBank", no: "901234567890", name: "CVLolos AI Indonesia" };

const PLANS = [
  {
    id: "free", label: "Gratis", desc: "Rp 0 / selamanya", price: 0, popular: false,
    features: ["1x Resume Builder", "Preview CV", "1x Download TXT/ATS"],
    locked: ["ATS Checker AI", "Cover Letter AI", "Career Guidance AI", "Unlimited Download"],
  },
  {
    id: "pro", label: "PRO", desc: `${fmt(49000)} / bulan`, price: 49000, popular: true,
    features: ["Semua fitur FREE", "ATS Checker AI Semantik", "Cover Letter AI Generator", "Career Guidance AI", "Unlimited Download PDF", "Prioritas Support"],
  },
  {
    id: "lifetime", label: "Lifetime", desc: `${fmt(149000)} sekali bayar`, price: 149000, popular: false,
    features: ["Semua fitur PRO", "Akses Selamanya", "Update Fitur Baru", "Badge Lifetime Member", "Akses Beta Fitur"],
  },
];
...
function Builder({ user, onUpdate }) {
  ...
  const handleDownload = () => {
    ...
    if (!isPro && user.downloads >= 1) {
      alert("Kamu sudah menggunakan jatah 1x download gratis. Upgrade ke PRO untuk download unlimited!");
      return;
    }
    ...
    a.download = `CV_${r.name.replace(/\s/g, "_")}_ATS.txt`;
    a.click();
  };
      ...
      <Card>
        ...
        {step === 5 && (
          <div className="space-y-4">
            ...
            {user.plan === "free" && <p className="text-xs text-center text-gray-400">Paket FREE: 1x download. <button className="text-blue-600 font-semibold">Upgrade PRO</button> untuk unlimited.</p>}
          </div>
        )}
      </Card>
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: `ATS Analyzer AI Indonesia. Analisis SEMANTIK (sinonim & makna). Return ONLY valid JSON: {"score":number,"keywords":[{"kw":"","found":"","type":"exact|semantic|missing"}],"format_issues":[{"issue":"","impact":"","fix":""}],"suggestions":[{"section":"","before":"","after":"","reason":""}],"summary":""}`,
      ...
      setResult({ score: 68, keywords: [{ kw: "React", found: "ReactJS", type: "semantic" }, { kw: "Agile", found: "Agile/Scrum", type: "exact" }, { kw: "Leadership", found: "memimpin tim", type: "semantic" }, { kw: "CI/CD", found: null, type: "missing" }, { kw: "Docker", found: null, type: "missing" }], format_issues: [{ issue: "Tabel di bagian skill", impact: "Tinggi — ATS tidak bisa membaca tabel", fix: "Ganti dengan teks biasa dipisah koma" }], suggestions: [{ section: "Ringkasan Profesional", before: "Saya developer berpengalaman dengan skill React.", after: "Frontend Developer dengan 2+ tahun pengalaman membangun web app menggunakan React.js, TypeScript & REST API.", reason: "Tambah angka pengalaman & keyword teknologi spesifik" }], summary: "CV cukup baik, namun beberapa keyword penting hilang dan ada masalah format yang perlu diperbaiki." });
    }
    setLoading(false);
  };
...
function CoverLetter({ user }) {
  ...
  const gen = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514", max_tokens: 1000,
          system: "Expert career coach Indonesia. Tulis cover letter profesional Bahasa Indonesia yang personal, tidak generik, ATS-friendly. Paragraf langsung tanpa header.",
          messages: [{ role: "user", content: `Nama: ${f.name}\nPosisi: ${f.pos}\nPerusahaan: ${f.company}\nJD: ${f.jd || "-"}\nTone: ${f.tone}` }]
        })
      });
      const d = await res.json();
      setLetter(d.content?.map(x => x.text || "").join("") || "");
    } catch {
      setLetter(`Kepada Yth. Tim Rekrutmen ${f.company},\n\nDengan hormat,\nSaya ${f.name}, dengan antusias mengajukan lamaran untuk posisi ${f.pos} di ${f.company}.\n\n[Konten lengkap akan di-generate oleh AI Claude]`);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      {letter && <Card className="space-y-3"><div className="flex justify-between items-center"><h3 className="font-bold text-gray-900">Cover Letter</h3><Btn sz="sm" v="secondary" onClick={() => navigator.clipboard.writeText(letter)}>📋 Copy</Btn></div><div className="bg-gray-50 rounded-xl p-4 text-sm text-gray-800 leading-relaxed whitespace-pre-line border border-gray-100 max-h-80 overflow-y-auto">{letter}</div></Card>}
    </div>
  );
}
  ...
  const analyze = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514", max_tokens: 1000,
          system: `Career coach AI Indonesia. Return ONLY valid JSON: {"paths":[{"title":"","prob":number,"why":""}],"gaps":[{"skill":"","priority":"high|medium|low","action":""}],"industries":[{"name":"","score":number,"reason":""}],"certs":[{"name":"","platform":"","duration":"","priority":"high|medium|low"}],"plan":{"m1":[],"m2":[],"m3":[]}}`,
          messages: [{ role: "user", content: `Pendidikan: ${f.edu}\nSkill: ${f.skills}\nPengalaman: ${f.exp}\nGoal: ${f.goal}\nIndustri: ${f.industry}` }]
        })
      });
      const d = await res.json();
      setResult(JSON.parse(d.content?.map(x => x.text || "").join("").replace(/```json|```/g, "").trim()));
    } catch {
      setResult({ paths: [{ title: "Frontend Developer", prob: 82, why: "Skill React & desain sangat relevan" }, { title: "UI/UX Designer", prob: 74, why: "Background seni & teknologi cocok" }, { title: "Product Manager", prob: 61, why: "Perlu tambah skill bisnis & data" }], gaps: [{ skill: "TypeScript", priority: "high", action: "Kursus TypeScript di Udemy (20 jam)" }, { skill: "Data Analytics", priority: "medium", action: "Google Data Analytics di Coursera" }], industries: [{ name: "Fintech", score: 85, reason: "Pertumbuhan tinggi, demand besar" }, { name: "E-commerce", score: 78, reason: "Banyak peluang remote" }], certs: [{ name: "Meta Frontend Developer", platform: "Coursera", duration: "7 bulan", priority: "high" }, { name: "Google UX Design", platform: "Coursera", duration: "6 bulan", priority: "high" }], plan: { m1: ["Buat portfolio project React", "Daftar kursus TypeScript", "Update CV & LinkedIn"], m2: ["Submit 10 lamaran ke fintech", "Mulai Meta Frontend Developer di Coursera"], m3: ["Target 3 interview undangan", "Selesaikan 1 sertifikasi"] } });
    }
    setLoading(false);
  };
  ...
  if (step === 4) return (
    <div className="max-w-lg mx-auto">
      <Card className="text-center py-10 space-y-4">
        ...
        <p className="text-gray-600 text-sm">Terima kasih! Tim kami akan memverifikasi dalam <strong>1×24 jam</strong>.</p>
    ...
    <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div>
            ...
            <div className="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-4 space-y-3">
              <div className="grid grid-cols-2 gap-y-2 text-sm">
                ...
                <div className="flex items-center gap-2"><span className="font-mono font-bold text-blue-700 text-base">{BANK.no}</span><button onClick={() => navigator.clipboard.writeText(BANK.no)} className="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded hover:bg-blue-200">Copy</button></div>
                ...
                <div className="flex items-center gap-2"><span className="font-bold text-green-700 text-base">{fmt(plan.price)}</span><button onClick={() => navigator.clipboard.writeText(String(plan.price))} className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded">Copy</button></div>
              </div>
      ...
      {step === 3 && (
        <Card className="space-y-4">
          ...
          <Input label="Nomor Referensi Transfer *" value={ref} onChange={setRef} placeholder="Contoh: TRF202501011234567" hint="Nomor transaksi dari m-banking kamu." />
...
function Admin() {
  ...
  return (
    <div className="space-y-5">
            ...
            <div className="space-y-2">
              {[...payments].reverse().map(p => {
                ...
                return <div key={p.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100">
                  ...
                  <div className="text-right flex items-center gap-2"><div><div className="font-bold text-sm text-blue-600">{fmt(p.amount)}</div><Badge c={p.status === "verified" ? "green" : p.status === "pending" ? "yellow" : "red"}>{p.status}</Badge></div>{p.status === "pending" && <div className="flex gap-1"><Btn sz="sm" v="success" onClick={() => verifyPayment(p.id)}>✓</Btn><Btn sz="sm" v="danger" onClick={() => rejectPayment(p.id)}>✗</Btn></div>}</div>
                </div>;
              })}
            </div>
          )}
      ...
      {tab === "settings" && (
        <Card className="space-y-4">
          <h3 className="font-bold text-gray-900">⚙️ Info Rekening & Pengaturan</h3>
          <Card className="bg-gray-50 border-dashed">
            <h4 className="text-sm font-bold text-gray-800 mb-2">📱 Info Rekening Penerima</h4>
            <div className="text-sm space-y-1 text-gray-700">
              <div><span className="font-semibold">Bank:</span> {BANK.bank}</div>
              <div><span className="font-semibold">No. Rekening:</span> {BANK.no}</div>
              <div><span className="font-semibold">Atas Nama:</span> {BANK.name}</div>
            </div>
          </Card>
          <p className="text-xs text-gray-400">Untuk mengubah harga atau info rekening, edit konstanta PLANS dan BANK di kode sumber.</p>
        </Card>
      )}
    </div>
  );
...
export default function App() {
  ...
  const updateUser = (u) => {
    setUser(u);
    const rec = _users.find(x => x.id === u.id);
    if (rec) Object.assign(rec, u);
  };

  const navigate = (p) => {
    if (PRO_PAGES.includes(p) && user?.plan === "free") {
      setActive("upgrade");
    } else {
      setActive(p);
    }
    ...
    <div className="min-h-screen bg-gray-50 flex">
        ...
        <div className="p-3 border-t border-gray-50">
          <div className="flex items-center gap-2 px-2 py-1.5 mb-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Badge c={user?.plan === "free" ? "gray" : user?.plan === "pro" ? "blue" : "purple"}>{user?.plan?.toUpperCase()}</Badge>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 md:hidden sticky top-0 z-20">
          ...
          <span className="font-bold text-gray-900 text-sm">CVLolos<span className="text-blue-600">.ai</span></span>
          <div className="ml-auto"><Badge c={user?.plan === "free" ? "gray" : user?.plan === "pro" ? "blue" : "purple"}>{user?.plan?.toUpperCase()}</Badge></div>
        </header>
